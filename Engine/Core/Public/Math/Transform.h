...
namespace SG
{
	...
	SG_FORCE_INLINE void Translate(Matrix4f& matrix, const Vector3f& pos)
	{
		matrix(0, 3) += pos(0);
		matrix(1, 3) += pos(1);
		matrix(2, 3) += pos(2);
	...
	SG_FORCE_INLINE void TranslateX(Matrix4f& matrix, float pos)
	{
		matrix(0, 3) += pos;
	}

	SG_FORCE_INLINE void TranslateY(Matrix4f& matrix, float pos)
	{
		matrix(1, 3) += pos;
	}

	SG_FORCE_INLINE void TranslateZ(Matrix4f& matrix, float pos)
	{
		matrix(2, 3) += pos;
	}

	SG_FORCE_INLINE void TranslateTo(Matrix4f& matrix, const Vector3f& pos)
	{
		matrix.col(3) << pos(0), pos(1), pos(2), 1.0f;
	}

	SG_FORCE_INLINE void TranslateToX(Matrix4f& matrix, float x)
	{
		matrix(0, 3) = x;
	}

	SG_FORCE_INLINE void TranslateToY(Matrix4f& matrix, float y)
	{
		matrix(1, 3) = y;
	}

	SG_FORCE_INLINE void TranslateToZ(Matrix4f& matrix, float z)
	{
		matrix(2, 3) = z;
	}

	SG_FORCE_INLINE void Scale(Matrix4f& matrix, const Vector3f& scale)
	{
		matrix(0, 0) *= scale(0);
		matrix(1, 1) *= scale(1);
		matrix(2, 2) *= scale(2);
	...
	SG_FORCE_INLINE void ScaleX(Matrix4f& matrix, float scale)
	{
		matrix(0, 0) *= scale;
	}

	SG_FORCE_INLINE void ScaleY(Matrix4f& matrix, float scale)
	{
		matrix(1, 1) *= scale;
	}

	SG_FORCE_INLINE void ScaleZ(Matrix4f& matrix, float scale)
	{
		matrix(2, 2) *= scale;
	}

	SG_FORCE_INLINE void ScaleTo(Matrix4f& matrix, const Vector3f& scale)
	{
		matrix(0, 0) = scale(0);
		matrix(1, 1) = scale(1);
		matrix(2, 2) = scale(2);
	...
	SG_FORCE_INLINE void RotateTo(Matrix4f& matrix, const Vector3f& rotation)
	{
	...
	SG_FORCE_INLINE Matrix4f BuildTransformMatrix(const Vector3f& position, float scale, const Vector3f& rotation)
	{
		Transform t = Translation(position) *
			AngleAxis(DegreesToRadians(rotation(0)), Vector3f{ 1.0f, 0.0f, 0.0f }) *
			AngleAxis(DegreesToRadians(rotation(1)), Vector3f{ 0.0f, 1.0f, 0.0f }) *
	...
	SG_FORCE_INLINE Vector3f PitchYawToUnitVector(const Vector3f& rotation)
	{
		Vector3f unitVec;
